...
SELECT
  ...
  SUM(CASE WHEN i.paid = true THEN i.total ELSE 0 END) as total_paid,
  SUM(CASE WHEN i.paid = false OR i.paid IS NULL THEN i.total ELSE 0 END) as total_pending,
  MAX(i.created_at) as last_invoice_date
FROM invoice_clients ic
LEFT JOIN invoices i ON i.client_id = ic.id
GROUP BY ic.id;
...
SELECT
  ...
  CASE
    WHEN mii.quantity_required > 0 THEN
      FLOOR(sp.current_stock / mii.quantity_required)
    ELSE 999999
...
SELECT
  ...
  s.restaurant_id,
  COALESCE(s.annual_leave_balance, 0) as annual_leave_balance,
  COUNT(CASE WHEN sl.status = 'approved' THEN 1 END) as approved_leaves,
  COUNT(CASE WHEN sl.status = 'pending' THEN 1 END) as pending_leaves,
  SUM(CASE
    WHEN sl.status = 'approved'
    THEN sl.end_date - sl.start_date + 1
    ELSE 0
  END) as total_days_taken
FROM staff s
LEFT JOIN staff_leave sl ON s.id = sl.staff_id
GROUP BY s.id, s.name, s.restaurant_id, s.annual_leave_balance;
