  ...
  if (!data || data.length === 0) {
    return (
      <div className="flex items-center justify-center h-64 bg-slate-50 rounded-xl">
        <p className="text-slate-400">{t('noProfitabilityData')}</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      {filteredData.length === 0 ? (
        <div className="flex items-center justify-center h-32 bg-slate-50 rounded-xl">
          <p className="text-slate-400">{t('noProductsFound').replace('{query}', searchQuery)}</p>
        </div>
      ) : (
        <>
          ...
          {hasMore && (
            ...
            >
              {t('showMore').replace('{count}', filteredData.length - displayCount)}
            </button>
          )}
        </>
      )}
