...
export default function AdminBilling() {
  ...
  return (
    <div>
      ...
      {/* SMS Verification Billing */}
      <h2 className="text-base font-bold text-slate-700 mb-3">SMS Verification Billing</h2>
      <div className="bg-white border-2 border-slate-100 rounded-2xl p-6 mb-10">
              ...
              <div className="border-2 border-slate-100 rounded-2xl overflow-hidden">
                <table className="w-full text-sm">
                  <thead className="bg-slate-50 border-b border-slate-100">
                    <tr>
                      <th className="text-left px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Venue</th>
                      <th className="text-center px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">SMS sent</th>
                      <th className="text-center px-5 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">Rate</th>
                  ...
                  <tbody>
                    {smsPreview.venues.map(v => (
                      <tr key={v.id} className="border-b border-slate-100 last:border-0 hover:bg-slate-50">
                        ...
                        <td className="px-5 py-3 text-center text-slate-600">{v.sms_count}</td>
                        <td className="px-5 py-3 text-center text-slate-500">{v.rate_pence}p</td>
                        <td className="px-5 py-3 text-right font-semibold text-slate-800">£{(v.amount_pence / 100).toFixed(2)}</td>
                        <td className="px-5 py-3 text-center">
                          {!v.billing_enabled ? (
                            <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-500">Opt-out</span>
                          ) : !v.has_stripe ? (
                            <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-700">No Stripe</span>
                          ) : (
                            <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-700">Will bill</span>
                          )}
                        </td>
                      </tr>
                    ))}
      ...
      <div className="bg-blue-50 border-2 border-blue-100 rounded-2xl p-6 mb-10">
