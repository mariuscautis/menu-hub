...
export default function StaffMembers() {
  ...
  if (loading) return <div className="text-slate-500">{t('loadingStaff')}</div>
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => { setShowModal(false); setIsEditing(false); setSelectedStaff(null); setError(null); setFormData({ email: '', name: '', role: 'staff', pin_code: '', department: departments[0] || 'kitchen', annual_holiday_days: 28.0, holiday_year_start: new Date().toISOString().split('T')[0], is_hub: false }) }}>
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-lg max-h-[92vh] flex flex-col shadow-2xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="overflow-y-auto flex-1 px-6 py-5">
              <form id="staff-form" onSubmit={isEditing ? editStaff : addStaff}>
                <div className="space-y-5">
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">{t('nameLabel')} <span className="text-red-500">*</span></label>
                    <input type="text" name="name" value={formData.name} onChange={handleChange} required className="w-full px-4 py-3 border-2 border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:border-[#6262bd] text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 placeholder:text-slate-400 dark:placeholder:text-slate-500 transition-colors" placeholder={formData.is_hub ? 'Hub Device Name' : t('namePlaceholder')}/>
                  </div>
                  ...
                  <label className="flex items-start gap-3 p-4 bg-slate-50 dark:bg-slate-800 border-2 border-slate-200 dark:border-slate-700 rounded-xl cursor-pointer hover:border-[#6262bd] transition-colors">
                    <input type="checkbox" id="is_hub" checked={formData.is_hub} onChange={(e) => setFormData({ ...formData, is_hub: e.target.checked })} className="mt-0.5 w-5 h-5 text-[#6262bd] border-slate-300 rounded focus:ring-[#6262bd]"/>
                    <div>
                      <p className="text-sm font-semibold text-slate-700 dark:text-slate-300">🍽️ Designate as Local Hub</p>
                      <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Coordinates local network sync between devices. Only one hub per restaurant.</p>
                    </div>
                  </label>
                  ...
                  {!formData.is_hub && (
                    <>
                      <div>
                        <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">{t('emailLabel')} <span className="text-red-500">*</span></label>
                        <div className="relative">
                          ...
                          <input type="email" name="email" value={formData.email} onChange={handleChange} required className="w-full pl-10 pr-4 py-3 border-2 border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:border-[#6262bd] text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 placeholder:text-slate-400 dark:placeholder:text-slate-500 transition-colors" placeholder={t('emailPlaceholder')}/>
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">{t('roleLabel')}</label>
                          <select name="role" value={formData.role} onChange={handleChange} className="w-full px-4 py-3 border-2 border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:border-[#6262bd] text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 transition-colors">
                            <option value="staff">{t('roleStaff')}</option>
                            <option value="admin">{t('roleAdmin')}</option>
                          </select>
                        </div>
                        <div>
                          <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">{t('departmentLabel')}</label>
                          <select name="department" value={formData.department} onChange={handleChange} className="w-full px-4 py-3 border-2 border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:border-[#6262bd] text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 transition-colors">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 dark:text-slate-300 mb-1.5">{t('pinCodeLabel')} {!isEditing && <span className="text-red-500">*</span>}</label>
                    <div className="flex gap-2">
                      <input type="text" name="pin_code" value={formData.pin_code} onChange={(e) => setFormData({ ...formData, pin_code: e.target.value.replace(/\D/g, '').slice(0, 3) })} required={!isEditing} maxLength={3} pattern="[0-9]{3}" inputMode="numeric" className="flex-1 px-4 py-3 border-2 border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:border-[#6262bd] text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 font-mono text-2xl text-center tracking-widest placeholder:text-slate-300 dark:placeholder:text-slate-600 transition-colors" placeholder="•••"/>
                      <button type="button" onClick={generatePinCode} className="px-4 py-3 bg-slate-100 dark:bg-slate-700 border-2 border-slate-200 dark:border-slate-600 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-600 font-medium text-sm whitespace-nowrap transition-colors">{t('generate')}</button>
                    </div>
                    <p className="text-slate-400 dark:text-slate-500 text-xs mt-1.5">{isEditing ? t('pinHintEdit') : t('pinHintAdd')}</p>
                  </div>
                  ...
                  {!formData.is_hub && (
                    <div className="pt-4 border-t-2 border-slate-100 dark:border-slate-700">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">{t('annualHolidayDays')}</label>
                          <input type="number" name="annual_holiday_days" value={formData.annual_holiday_days} onChange={handleChange} step="0.5" min="0" max="365" className="w-full px-4 py-2.5 border-2 border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:border-[#6262bd] text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 transition-colors"/>
                          <p className="text-slate-400 dark:text-slate-500 text-xs mt-1">{t('annualHolidayDaysHint')}</p>
                        </div>
                        <div>
                          <label className="block text-xs font-medium text-slate-600 dark:text-slate-400 mb-1.5">{t('holidayYearStartDate')}</label>
                          <input type="date" name="holiday_year_start" value={formData.holiday_year_start} onChange={handleChange} className="w-full px-4 py-2.5 border-2 border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:border-[#6262bd] text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 transition-colors"/>
            ...
            <div className="px-6 py-4 border-t-2 border-slate-100 dark:border-slate-700 flex gap-3 flex-shrink-0">
              <button type="button" onClick={() => { setShowModal(false); setIsEditing(false); setError(null) }} className="flex-1 border-2 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 py-3 rounded-xl font-medium hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">{t('cancel')}</button>
              <button type="submit" form="staff-form" className="flex-1 bg-[#6262bd] text-white py-3 rounded-xl font-semibold hover:bg-[#5252a3] transition-colors shadow-sm">{isEditing ? t('updateStaff') : t('addStaffButton')}</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4" onClick={() => { setShowPasswordModal(false); setPasswordData({ newPassword: '' }); setSelectedStaff(null) }}>
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-sm shadow-2xl" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between px-6 py-5 border-b-2 border-slate-100 dark:border-slate-700">
              ...
              <button onClick={() => { setShowPasswordModal(false); setSelectedStaff(null); setPasswordData({ newPassword: '' }) }} className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700 text-xl font-bold transition-colors">×</button>
            </div>
            <form onSubmit={updatePinCode} className="px-6 py-5">
              ...
              <input type="text" value={passwordData.newPassword} onChange={(e) => setPasswordData({ newPassword: e.target.value.replace(/\D/g, '').slice(0, 3) })} maxLength={3} pattern="[0-9]{3}" inputMode="numeric" className="w-full px-4 py-3 border-2 border-slate-200 dark:border-slate-700 rounded-xl focus:outline-none focus:border-[#6262bd] text-slate-700 dark:text-slate-200 bg-white dark:bg-slate-800 font-mono text-xl text-center tracking-widest placeholder:text-slate-300 transition-colors" placeholder="Enter 3-digit PIN" autoFocus/>
              <div className="flex gap-3 mt-5">
                <button type="button" onClick={() => { setShowPasswordModal(false); setSelectedStaff(null); setPasswordData({ newPassword: '' }) }} className="flex-1 border-2 border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 py-3 rounded-xl font-medium hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">{t('cancel')}</button>
                <button type="submit" disabled={passwordData.newPassword.length !== 3} className="flex-1 bg-[#6262bd] text-white py-3 rounded-xl font-semibold hover:bg-[#5252a3] disabled:opacity-40 disabled:cursor-not-allowed transition-colors">{t('updatePin')}</button>
              </div>
            </form>
          </div>
        </div>
