...
export default function OtherOptionsSettings() {
  ...
  // Hub device state
  const [isThisDeviceHub, setIsThisDeviceHub] = useState(false)
  ...
  // Detect whether this device is marked as the hub, and whether hub server is reachable
  useEffect(() => {
    if (typeof window === 'undefined') return

    // Check localStorage flag (set per-device, not per-restaurant)
    const isHub = localStorage.getItem('menuhub_is_hub_device') === 'true'
    setIsThisDeviceHub(isHub)

    // Try to get local IPs by connecting to a dummy WebRTC peer
    // (no STUN needed — reflexive candidates from local network interfaces)
    try {
      ...
      // Fallback: close after 3s regardless
      setTimeout(() => {
        pc.close()
        setLocalIPs(Array.from(ips))
      }, 3000)
    } catch {
      // RTCPeerConnection unavailable — skip
    }

    // Check if hub server is reachable on localhost:3001
    if (isHub) {
      checkHubServer()
    }
  ...
  const checkHubServer = async () => {
    setHubCheckLoading(true)
    try {
      const res = await fetch('http://localhost:3001/health', {
        signal: AbortSignal.timeout(2000),
      })
      setHubRunning(res.ok)
    } catch {
      setHubRunning(false)
    }
  ...
  const handleToggleHubDevice = async (enable) => {
    setSavingHub(true)
    localStorage.setItem('menuhub_is_hub_device', enable ? 'true' : 'false')
    setIsThisDeviceHub(enable)

    // Save hub IP(s) to restaurant settings in Supabase so other devices can discover it
    if (enable && restaurant) {
      const hubIp = localIPs[0] || null
      await supabase
        .from('restaurants')
        .update({ hub_settings: { enabled: true, ip: hubIp, port: 3001, updatedAt: new Date().toISOString() } })
        .eq('id', restaurant.id)
    } else if (!enable && restaurant) {
      await supabase
        .from('restaurants')
        .update({ hub_settings: { enabled: false, ip: null, port: 3001, updatedAt: new Date().toISOString() } })
        .eq('id', restaurant.id)
    }

    if (enable) {
      // Check if hub server is already running
      await checkHubServer()
    } else {
      setHubRunning(false)
    }
      ...
      <div className="bg-white border-2 border-slate-100 rounded-2xl p-6 mb-6">
        <div className="flex items-start justify-between gap-4">
          <div className="flex-1">
            ...
            <p className="text-sm text-slate-500">
              Mark this device as the local hub. All other devices on the same WiFi will connect to it automatically, enabling real-time order sync without internet.
            </p>
          </div>
          <label className="relative inline-flex items-center cursor-pointer flex-shrink-0 mt-1">
            <input
              type="checkbox"
              checked={isThisDeviceHub}
              onChange={(e) => handleToggleHubDevice(e.target.checked)}
              disabled={savingHub}
              className="sr-only peer"
            />
            <div className="w-11 h-6 bg-slate-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-[#6262bd]/20 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-slate-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-[#6262bd] disabled:opacity-50"></div>
            <span className="ml-3 text-sm font-medium text-slate-700">
              {savingHub ? 'Saving...' : isThisDeviceHub ? 'This is the Hub' : 'Not the Hub'}
            </span>
          </label>
        </div>
        ...
        {isThisDeviceHub && (
          <div className="mt-5 space-y-4 border-t border-slate-100 pt-5">
            ...
            <div className="flex items-center justify-between p-3 bg-slate-50 rounded-xl">
              <div className="flex items-center gap-2">
                <div className={`w-2.5 h-2.5 rounded-full ${hubRunning ? 'bg-green-500 animate-pulse' : 'bg-red-400'}`} />
                <span className="text-sm font-medium text-slate-700">
                  Hub server: {hubCheckLoading ? 'Checking...' : hubRunning ? 'Running on port 3001' : 'Not running'}
                </span>
              </div>
              <button
                onClick={checkHubServer}
                disabled={hubCheckLoading}
            ...
            {localIPs.length > 0 && (
              <div className="p-3 bg-blue-50 border border-blue-100 rounded-xl">
                <p className="text-xs font-medium text-blue-700 mb-1">This device&apos;s local IP{localIPs.length > 1 ? 's' : ''}:</p>
                {localIPs.map(ip => (
                  <p key={ip} className="text-sm font-mono text-blue-800">ws://{ip}:3001</p>
                ))}
                <p className="text-xs text-blue-600 mt-1">Other devices on the same WiFi will find this hub automatically.</p>
              </div>
            )}
            ...
            {!hubRunning && (
              <div className="p-4 bg-amber-50 border border-amber-100 rounded-xl">
                <p className="text-sm font-semibold text-amber-800 mb-2">Start the hub server</p>
                <p className="text-xs text-amber-700 mb-3">
                  The hub server is a small Node.js process that must be running on this device. Open a terminal and run:
                </p>
                <div className="bg-amber-900 rounded-lg p-3 font-mono text-xs text-amber-100 space-y-1">
                  <p>cd hub-server</p>
                  <p>npm install</p>
                  <p>npm start</p>
                </div>
                <p className="text-xs text-amber-600 mt-2">
                  Keep the terminal open. The server runs on port 3001.
                </p>
              </div>
            )}
              ...
              <div className="p-3 bg-green-50 border border-green-100 rounded-xl">
                <div className="flex gap-2">
                  ...
                  <div className="text-sm text-green-800">
                    <p className="font-medium">Hub is active</p>
                    <p className="text-xs text-green-700 mt-0.5">All devices on this WiFi network can now connect automatically and sync orders in real time — even without internet.</p>
                  </div>
                </div>
              </div>
            )}
          ...
          <div className="mt-4 p-3 bg-slate-50 border border-slate-100 rounded-xl">
            <div className="flex gap-2">
              ...
              <p className="text-xs text-slate-500">
                Enable this on the <strong>lobby or bar tablet</strong> — the device that stays on all day. Staff devices (waiters, kitchen screens) do not need this toggle; they connect automatically.
              </p>
            </div>
          </div>
        )}
