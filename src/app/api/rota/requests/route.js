...
async function sendRequestAmendedEmail(supabase, { requestId }) {
  try {
    const { data: req } = await supabase
      .from('shift_requests')
      .select('*, staff:staff_id(name, email), restaurant:restaurant_id(name, email_language)')
    ...
    const subject = t(tr.requestAmendedSubject, { restaurantName })
    const html = `<!DOCTYPE html>
<html lang="en">
<head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"></head>
<body style="margin:0;padding:0;background-color:#f4f4f8;font-family:Arial,Helvetica,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background-color:#f4f4f8;padding:32px 0;">
    <tr><td align="center">
      <table width="600" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;overflow:hidden;box-shadow:0 4px 16px rgba(0,0,0,0.08);max-width:600px;width:100%;">
        <tr>
          <td style="background-color:#6262bd;padding:40px 32px;text-align:center;">
            <p style="margin:0 0 8px 0;font-size:13px;color:rgba(255,255,255,0.85);letter-spacing:1px;text-transform:uppercase;">${restaurantName}</p>
            <h1 style="margin:0;font-size:26px;font-weight:700;color:#ffffff;">✏️ ${tr.requestAmendedTitle}</h1>
          </td>
        </tr>
        <tr>
          <td style="padding:36px 32px;background-color:#ffffff;">
            <p style="margin:0 0 16px 0;font-size:16px;color:#1f2937;">${t(tr.requestAmendedGreeting, { staffName })}</p>
            <p style="margin:0 0 24px 0;font-size:15px;color:#4b5563;line-height:1.6;">${tr.requestAmendedBody}</p>
            <table width="100%" cellpadding="0" cellspacing="0" style="background:#f0f0fb;border-radius:10px;border:1px solid #d0d0f0;margin-bottom:28px;">
              <tr>
                <td style="padding:14px 20px;border-bottom:1px solid #d0d0f0;">
                  <span style="font-size:12px;font-weight:700;color:#6262bd;text-transform:uppercase;">${tr.requestAmendedDates}</span>
                  <p style="margin:4px 0 0 0;font-size:15px;color:#374151;">${dateRange}</p>
                </td>
              </tr>
            </table>
            <div style="text-align:center;margin:8px 0 32px 0;">
              <a href="${rotaUrl}" style="display:inline-block;padding:14px 32px;background-color:#6262bd;color:#ffffff;text-decoration:none;border-radius:8px;font-size:15px;font-weight:600;">${tr.requestAmendedCta} →</a>
            </div>
            <p style="margin:0;font-size:13px;color:#9ca3af;">${t(tr.requestAmendedFooter, { restaurantName })}</p>
          </td>
        </tr>
      </table>
    </td></tr>
    ...
    console.error('Failed to send request amended email:', err)
  }
}
